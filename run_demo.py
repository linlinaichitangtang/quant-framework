...
def generate_simulated_data(n_stocks: int = 50, n_days: int = 1500, random_seed: int = 42):
    """生成模拟数据用于测试"""
...
def main():
    print("=" * 60)
    print("基于机器学习的超短线尾盘战法 - 演示运行（模拟数据）")
    print("=" * 60)
    ...
    # 1. 生成模拟数据
    print("\n[Step 1] 生成模拟数据...")
    combined_df = generate_simulated_data(n_stocks=50, n_days=1000)
    print(f"生成了 {len(combined_df['ts_code'].unique())} 只股票，共 {len(combined_df)} 个交易日数据")
    ...
    # 3. Optuna超参数优化（使用较少的trials）
    print("\n[Step 3] Optuna超参数优化 (简化版，只跑20次)...")
    # 分割训练集
    ...
    optimizer = OptunaOptimizer(X_train, y_train, model_type='gbm', n_splits=3)
    best_params = optimizer.optimize(n_trials=20, show_progress_bar=True)
    
    pd.DataFrame([best_params]).to_csv(os.path.join(output_dir, 'best_params.csv'), index=False)
    ...
    if not fi_shap.empty:
        ...
        analyzer.dependence_plot(
            top_feature, 
            os.path.join(output_dir, f'shap_dependence_{top_feature}.png')
        )
    ...
    backtest_dates = combined_df[
        (combined_df['trade_date'].astype(int) >= int(backtest_start)) & 
        (combined_df['trade_date'].astype(int) <= int('20260315'))
    ...
    with open(os.path.join(output_dir, 'backtest_report.txt'), 'w') as f:
        f.write("基于机器学习的超短线尾盘战法回测报告 (模拟数据演示)\n")
        f.write("=" * 60 + "\n")
